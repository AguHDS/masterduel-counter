...
- **Tier List**: Scraping de masterduelmeta.com (MD) y yugiohmeta.com (TCG/OCG) cada 12h. Muestra decks meta en tiers 1-4 (TCG/OCG: top 3 = T1, ≥2% = T2, ≥1% = T3, <1% = T4; MD: T4 = trending/long-tail). T0 solo manual (entry manual con tier 0, congelada). Linking de entries a archetypes de la DB (scraped linkeadas siguen al meta; manuales quedan congeladas aunque esten linkeadas). Soft-delete + auto-reset (solo scraped) + restore UI via GET /api/tier-list/inactive. Imagen y arquetipo linkeado se sincronizan entre los 3 formatos (mismo deckName). Imagenes resueltas via selectCard (DB local) → YGOProDeck (hotlink fallback). Ver `.docs/tierlist.md`.
