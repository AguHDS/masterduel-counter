...
- **Tier List**: Scraping de masterduelmeta.com (MD) y yugiohmeta.com (TCG/OCG) cada 12h. Muestra decks meta en tiers 1-4 (TCG/OCG: top 3 = T1, ≥2% = T2, ≥1% = T3, <1% = T4; MD: T4 = trending/long-tail). Linking de entries a archetypes de la DB. Soft-delete + auto-reset. Imagen y arquetipo linkeado se sincronizan entre los 3 formatos (mismo deckName). Imagenes resueltas via selectCard (DB local) → YGOProDeck (hotlink fallback). Ver `.docs/tierlist.md`.
